...
2）接着数据被发送到IP层ip_local_deliver_finish()，经过剥离IP头部，把数据往TCP层发

3）tcp层tcp_v4_rcv()接收到数据后，再调用tcp_rcv_established()处理(ps:ack也是其中的tcp_ack()发送的)，如果当前用户进程没有正在读取数据，则将其插入到接收队列中，tcp_queue_rcv()则判断接收队列是否为空，如果不为空，则将skb合并到接收队列的尾部，最后由tcp_recvmsg()从接收列队中一个一个的将skb读取到用户设置的buffer中去。
